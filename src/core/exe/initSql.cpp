...
void createTable(){
    ...
    stmt->execute("CREATE TABLE `simhash` (`id` int(30) NOT NULL AUTO_INCREMENT,`simhash` varchar(128) NOT NULL,`docIds` text NOT NULL,`createTime` datetime NOT NULL,`updateTime` datetime NOT NULL,PRIMARY KEY (`id`)) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;");

    stmt->execute("DROP TABLE IF EXISTS `word`");
    stmt->execute("CREATE TABLE `word` (`id` int(20) NOT NULL AUTO_INCREMENT,`word` varchar(255) NOT NULL,`postingList` longtext NOT NULL,`docCount` int(30) NOT NULL,`totalCount` int(30) NOT NULL,PRIMARY KEY (`id`)) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4");
}
